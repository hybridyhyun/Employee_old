...
public class SpringJdbcDeptDao implements DeptDao {
	...
	class DeptResultSetMyExtractor implements ResultSetExtractor<Dept>{
	...
	public Dept selectByDeptnoWithEmps(Integer deptno){
		...
		JdbcTemplate template = new JdbcTemplate(dataSource);
		DeptResultSetMyExtractor rse = new DeptResultSetMyExtractor();
		
		Dept dept = template.query(SELECT_BY_DEPTNO_WITH_EMPS, new Object[] {deptno},
				rse);
